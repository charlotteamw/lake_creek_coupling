...
# Shared theme + colours (used by both isotopes and telemetry)
...
fill_scheme <- c("creek" = "#3A74B4",   # Creek
                 "lake"  = "#D1775E")   # Lake
...
calculate_prop_tp <- function(iso_data, chosen_species, chosen_tissue) {
  ...
  for (month in months) {
    df <- iso_data %>%
      filter(organism %in% chosen_species,
    ...
    # Trophic position (1 + weighted baseline offsets)
...
# ------------------------------
# Compute metrics for pooled shiners (liver)
# ------------------------------
...
# ------------------------------
# Isotope summaries (by month × location)
# ------------------------------
...
# ------------------------------
# Drop fish with no overlap between tag window and detections
# ------------------------------
...
create_time_series <- function(data, id_time) {
  ...
  if (nrow(fish_data) == 0 ||
    ...
    warning(paste("Skipping id_time:", id_time,
                  "due to missing or invalid dates"))
    return(NULL)
  }
...
for (fish_id in unique(time_series_all$id_time)) {
  ...
  for (i in seq_len(nrow(fish_data))) {
    # Exit: first non-lake after being in lake
    if (!is.na(fish_data$location[i]) &&
        fish_data$location[i] != "lake" &&
        !exited) {
      exited         <- TRUE
      last_exit_time <- fish_data$date_time[i]
    }
    
    # Return: back into lake after an exit
    if (!is.na(fish_data$location[i]) &&
        fish_data$location[i] == "lake" &&
        exited) {
      ...
      if (!is.na(last_exit_time)) {
        interval <- as.numeric(
          difftime(fish_data$date_time[i], last_exit_time, units = "mins")
        )
...
# ------------------------------
# Telemetry summaries (by Season × release_location)
# ------------------------------
...
plot_residency <- ggplot(telemetry_summary_by_loc,
  ...
  geom_errorbar(aes(ymin = mean_lake_residency - se_lake_residency,
                ...
                width = 0.15, size = 0.6,
                color = "black", alpha = 0.7) +
  geom_point(aes(fill = release_location),
             size = 3, shape = 21,
             color = "grey20", stroke = 0.6) +
  scale_fill_manual(name   = "Location",
                    values = fill_scheme,
...
# =============================================================================
# PART 3: FIGURE 3 – 2×2 GRID (ISOTOPES LEFT, TELEMETRY RIGHT)
# =============================================================================

# First, combine isotopes into a left column (no legend here)
left_iso <- ggarrange(
...
# Combine telemetry into a right column (no legend here)
right_tel <- ggarrange(
...
# Create a dummy plot to extract a shared legend
dummy_for_legend <- plot_shiner_carbon
...
# Combine left and right columns into top row
top_row <- ggarrange(
  left_iso,
  right_tel,
  ncol = 2, nrow = 1,
...
# Final Figure 3: top row + shared legend
figure_3 <- ggarrange(
  top_row,
  legend_shared,
  ncol = 1,
