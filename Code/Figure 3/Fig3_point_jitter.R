...
# Shared dodge position — same object ensures perfect alignment across layers
...
# ------------------------------
transmitters_to_remove <- c("34905", "32999", "42348", "32993")
...
# ------------------------------
# Recurrence (return to first detected habitat) — seasonal
# ------------------------------
...
# ------------------------------
# Recurrence count per individual (across full deployment)
# ------------------------------
...
for (fish_id in unique(time_series_all$id_time)) {
  ...
  for (i in seq_len(nrow(fish_data))) {
    this_loc <- fish_data$location[i]
    if (is.na(this_loc)) next
    if (this_loc != first_loc && !exited) exited <- TRUE
...
# -----------------------------------------------
# Add numeric x + manual offset to summary data
# -----------------------------------------------

# For isotopes
shiners_summary <- shiners_summary %>%
  mutate(
    x_num    = as.numeric(month),
    x_nudged = case_when(location == "creek" ~ x_num - 0.1,
                         location == "lake"  ~ x_num + 0.1)
  )

shiners_liver <- shiners_liver %>%
  mutate(
    x_num    = as.numeric(month),
    x_nudged = case_when(location == "creek" ~ x_num - 0.1,
                         location == "lake"  ~ x_num + 0.1)
...
# For telemetry
telemetry_summary_by_loc <- telemetry_summary_by_loc %>%
  mutate(
    x_num    = as.numeric(Season),
    x_nudged = case_when(release_location == "creek" ~ x_num - 0.1,
                         release_location == "lake"  ~ x_num + 0.1)
  )
...
plot_shiner_carbon <- ggplot() +
  geom_point(
    data  = shiners_liver,
    aes(x = x_nudged, y = lake_carbon, fill = location),
    size = 1.5, shape = 21, color = "grey20", stroke = 0.3, alpha = 0.2
  ) +
  geom_errorbar(
    data  = shiners_summary,
    aes(x = x_nudged, ymin = avg_lake_carbon - se_lake_carbon,
        ymax = avg_lake_carbon + se_lake_carbon),
    width = 0.1, size = 0.6, color = "black", alpha = 0.7
  ) +
  geom_point(
    data  = shiners_summary,
    aes(x = x_nudged, y = avg_lake_carbon, fill = location),
    size = 3, shape = 21, color = "grey20", stroke = 0.6
  ) +
  scale_x_continuous(breaks = 1:3, labels = c("Spring", "Summer", "Fall")) +
...
plot_shiner_tp <- ggplot() +
  geom_point(
    data  = shiners_liver,
    aes(x = x_nudged, y = trophic_position, fill = location),
    size = 1.5, shape = 21, color = "grey20", stroke = 0.3, alpha = 0.2
  ) +
  geom_errorbar(
    data  = shiners_summary,
    aes(x = x_nudged, ymin = avg_tp - se_tp,
        ymax = avg_tp + se_tp),
    width = 0.1, size = 0.6, color = "black", alpha = 0.7
  ) +
  geom_point(
    data  = shiners_summary,
    aes(x = x_nudged, y = avg_tp, fill = location),
    size = 3, shape = 21, color = "grey20", stroke = 0.6
  ) +
  scale_x_continuous(breaks = 1:3, labels = c("Spring", "Summer", "Fall")) +
  scale_fill_manual(name = "Location", values = fill_scheme,
...
plot_residency <- ggplot() +
  geom_point(
    data  = final_results,
    aes(x = x_nudged, y = lake_residency, fill = release_location),
    size = 1.5, shape = 21, color = "grey20", stroke = 0.3, alpha = 0.2
  ) +
  geom_errorbar(
    data  = telemetry_summary_by_loc,
    aes(x = x_nudged, ymin = mean_lake_residency - se_lake_residency,
        ymax = mean_lake_residency + se_lake_residency),
    width = 0.1, size = 0.6, color = "black", alpha = 0.7
  ) +
  geom_point(
    data  = telemetry_summary_by_loc,
    aes(x = x_nudged, y = mean_lake_residency, fill = release_location),
    size = 3, shape = 21, color = "grey20", stroke = 0.6
  ) +
  scale_x_continuous(breaks = 1:3, labels = c("Spring", "Summer", "Fall")) +
  scale_fill_manual(name = "Location", values = fill_scheme,
...
plot_recurrence <- ggplot() +
  geom_point(
    data  = final_results,
    aes(x = x_nudged, y = total_returns, fill = release_location),
    size = 1.5, shape = 21, color = "grey20", stroke = 0.3, alpha = 0.2
  )  +
  geom_errorbar(
    data  = telemetry_summary_by_loc,
    aes(x = x_nudged, ymin = mean_recurrence - se_recurrence,
        ymax = mean_recurrence + se_recurrence),
    width = 0.1, size = 0.6, color = "black", alpha = 0.7
  ) +
  geom_point(
    data  = telemetry_summary_by_loc,
    aes(x = x_nudged, y = mean_recurrence, fill = release_location),
    size = 3, shape = 21, color = "grey20", stroke = 0.6
  ) +
  scale_x_continuous(breaks = 1:3, labels = c("Spring", "Summer", "Fall")) +
  scale_fill_manual(name = "Location", values = fill_scheme,
