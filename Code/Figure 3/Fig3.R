...
plot_shiner_carbon <- ggplot(shiners_summary,
  ...
  geom_point(aes(fill = location), size = 3, shape = 21, color = "#323332", stroke = 0.6) +
...
# =============================================================================
# RESULTS VERIFICATION – Isotopes + Residency + Recurrence
# =============================================================================
...
# -------------------------------------------------------
# 1. BASELINE ISOTOPE STATS
# -------------------------------------------------------

baselines <- isotope_data %>%
  filter(organism %in% c("mayfly", "mussel"))

# Mayfly stats by location
mayfly_stats <- isotope_data %>%
  filter(organism == "mayfly") %>%
  group_by(location) %>%
  summarise(
    n        = n(),
    mean_d13C = round(mean(d13C, na.rm = TRUE), 2),
    sd_d13C   = round(sd(d13C,   na.rm = TRUE), 2),
...
# PERMANOVA – mayflies (location, season, year)
mayfly_data <- isotope_data %>%
  filter(organism == "mayfly") %>%
  filter(!is.na(d13C) & !is.na(d15N) & !is.na(location) & !is.na(month) & !is.na(year))

mayfly_matrix <- as.matrix(mayfly_data[, c("d13C", "d15N")])

cat("\n--- PERMANOVA: Mayfly ~ location ---\n")
print(adonis2(mayfly_matrix ~ location, data = mayfly_data, permutations = 999))

cat("\n--- PERMANOVA: Mayfly ~ month (season) ---\n")
print(adonis2(mayfly_matrix ~ month, data = mayfly_data, permutations = 999))

cat("\n--- PERMANOVA: Mayfly ~ year ---\n")
print(adonis2(mayfly_matrix ~ as.factor(year), data = mayfly_data, permutations = 999))

# PERMANOVA – mussels
mussel_data <- isotope_data %>%
  filter(organism == "mussel") %>%
  filter(!is.na(d13C) & !is.na(d15N) & !is.na(location) & !is.na(month) & !is.na(year))

mussel_matrix <- as.matrix(mussel_data[, c("d13C", "d15N")])

cat("\n--- PERMANOVA: Mussel ~ location ---\n")
print(adonis2(mussel_matrix ~ location, data = mussel_data, permutations = 999))

cat("\n--- PERMANOVA: Mussel ~ month (season) ---\n")
print(adonis2(mussel_matrix ~ month, data = mussel_data, permutations = 999))

cat("\n--- PERMANOVA: Mussel ~ year ---\n")
print(adonis2(mussel_matrix ~ as.factor(year), data = mussel_data, permutations = 999))

# -------------------------------------------------------
# 2. SHINER RAW ISOTOPE STATS BY SPECIES & LOCATION
# -------------------------------------------------------

shiner_iso_stats <- isotope_data %>%
  filter(organism %in% c("golden shiner", "common shiner"), tissue == "liver") %>%
...
for (sp in c("golden shiner", "common shiner")) {
  sp_data <- isotope_data %>%
    filter(organism == sp, tissue == "liver") %>%
    filter(!is.na(d13C) & !is.na(d15N) & !is.na(location) & !is.na(month) & !is.na(year))
  
  sp_matrix <- as.matrix(sp_data[, c("d13C", "d15N")])
  
  cat(sprintf("\n--- PERMANOVA: %s ~ location ---\n", sp))
  print(adonis2(sp_matrix ~ location, data = sp_data, permutations = 999))
  
  cat(sprintf("\n--- PERMANOVA: %s ~ month (season) ---\n", sp))
  print(adonis2(sp_matrix ~ month, data = sp_data, permutations = 999))
  
  cat(sprintf("\n--- PERMANOVA: %s ~ year ---\n", sp))
  print(adonis2(sp_matrix ~ as.factor(year), data = sp_data, permutations = 999))
}

# -------------------------------------------------------
# 3. MIXING MODEL – ANOVA + TUKEY on lake-derived carbon
# -------------------------------------------------------


# Two-way ANOVA: season * location
anova_model <- aov(lake_carbon ~ month * location, data = shiners_liver)
cat("\n--- Two-way ANOVA: lake_carbon ~ season * location ---\n")
...
# Mean ± SE by season x location (for delta calculations)
mixing_summary <- shiners_liver %>%
  group_by(month, location) %>%
  summarise(
    n    = n(),
    mean = round(mean(lake_carbon, na.rm = TRUE), 3),
    se   = round(standard_error(lake_carbon), 3),
    sd   = round(sd(lake_carbon, na.rm = TRUE), 3),
    .groups = "drop"
  )
cat("\n--- Mean ± SE lake-derived carbon by season & location ---\n")
print(mixing_summary)

# Delta calculations between seasons within each location
cat("\n--- Delta values (season-to-season changes) ---\n")

for (loc in c("creek", "lake")) {
  loc_data <- mixing_summary %>% filter(location == loc)
  spr <- loc_data %>% filter(month == "Spring") %>% pull(mean)
  sum <- loc_data %>% filter(month == "Summer") %>% pull(mean)
  fal <- loc_data %>% filter(month == "Fall")   %>% pull(mean)
...
# Delta between lake and creek per season
cat("\n--- Delta: lake vs creek per season ---\n")
for (seas in c("Spring", "Summer", "Fall")) {
  lk_mean <- mixing_summary %>% filter(month == seas, location == "lake")  %>% pull(mean)
  cr_mean <- mixing_summary %>% filter(month == seas, location == "creek") %>% pull(mean)
  cat(sprintf("  %s – lake minus creek: Δ = %+.3f\n", seas, lk_mean - cr_mean))
}

library(rcompanion)  # for scheirerRayHare

# -------------------------------------------------------
# 4. LAKE RESIDENCY – two-way Scheirer-Ray-Hare
# -------------------------------------------------------

cat("\n--- Lake Residency: Scheirer-Ray-Hare (Season × release_location) ---\n")
srh_residency <- scheirerRayHare(
  lake_residency ~ Season + release_location + Season:release_location,
  data = final_results_all_seasons
)
print(srh_residency)

# Post-hoc: Dunn's on Season (collapsed across location)
cat("\nDunn's Post-hoc – Residency by Season (Bonferroni):\n")
dunn.test(
  x      = final_results_all_seasons$lake_residency,
  g      = final_results_all_seasons$Season,
  method = "bonferroni",
  kw     = FALSE
)

# Post-hoc: Dunn's on release_location (collapsed across season)
cat("\nDunn's Post-hoc – Residency by Release Location (Bonferroni):\n")
dunn.test(
  x      = final_results_all_seasons$lake_residency,
  g      = final_results_all_seasons$release_location,
  method = "bonferroni",
  kw     = FALSE
)

# If interaction is significant: pairwise within each Season
cat("\nDunn's Post-hoc – Residency by Season × Release Location (interaction):\n")
final_results_all_seasons <- final_results_all_seasons %>%
  mutate(season_loc = paste(Season, release_location, sep = "_"))

dunn.test(
  x      = final_results_all_seasons$lake_residency,
  g      = final_results_all_seasons$season_loc,
  method = "bonferroni",
  kw     = FALSE
)

# Summary table
cat("\n--- Mean ± SE: Residency by Season × Release Location ---\n")
final_results_all_seasons %>%
  group_by(Season, release_location) %>%
  summarise(
    n    = n(),
    mean = round(mean(lake_residency, na.rm = TRUE), 3),
    se   = round(standard_error(lake_residency), 3),
    .groups = "drop"
  ) %>% print()

# -------------------------------------------------------
# 5. LAKE RECURRENCE – two-way Scheirer-Ray-Hare
# -------------------------------------------------------

cat("\n--- Lake Recurrence: Scheirer-Ray-Hare (Season × release_location) ---\n")
srh_recurrence <- scheirerRayHare(
  total_returns ~ Season + release_location + Season:release_location,
  data = final_results_all_seasons
)
print(srh_recurrence)

# Post-hoc: Dunn's on Season
cat("\nDunn's Post-hoc – Recurrence by Season (Bonferroni):\n")
dunn.test(
  x      = final_results_all_seasons$total_returns,
  g      = final_results_all_seasons$Season,
  method = "bonferroni",
  kw     = FALSE
)

# Post-hoc: Dunn's on release_location
cat("\nDunn's Post-hoc – Recurrence by Release Location (Bonferroni):\n")
dunn.test(
  x      = final_results_all_seasons$total_returns,
  g      = final_results_all_seasons$release_location,
  method = "bonferroni",
...
# If interaction is significant: pairwise within each Season × Location combo
cat("\nDunn's Post-hoc – Recurrence by Season × Release Location (interaction):\n")
dunn.test(
  x      = final_results_all_seasons$total_returns,
  g      = final_results_all_seasons$season_loc,
  method = "bonferroni",
...
# Summary table
cat("\n--- Mean ± SE: Recurrence by Season × Release Location ---\n")
final_results_all_seasons %>%
  group_by(Season, release_location) %>%
  summarise(
    n    = n(),
    mean = round(mean(total_returns, na.rm = TRUE), 1),
    se   = round(standard_error(total_returns), 1),
    .groups = "drop"
  ) %>% print()
